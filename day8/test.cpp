...
namespace day8
{
	...
	TEST(ParseLines, Example1) {
		auto lines = read_lines("./day8/input/example1.txt");
	...
	TEST(Solution, Example2) {
		auto lines = read_lines("./day8/input/example2.txt");
		Input input{ .map = parse_lines(lines) };
		EXPECT_EQ(6, input.solve());
	}
	TEST(Solution, Example3Part2) {
		auto lines = read_lines("./day8/input/example3.txt");
		Input input{ .map = parse_lines(lines) };
		EXPECT_EQ(6, input.solve_ghosts_naive());
		EXPECT_EQ(6, input.solve_ghosts());
	}
	TEST(Solution, Input) {
		auto lines = read_lines("./day8/input/input.txt");
		Input input{ .map = parse_lines(lines) };
